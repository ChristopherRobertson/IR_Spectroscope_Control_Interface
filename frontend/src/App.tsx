...
const PlaceholderView: React.FC<PlaceholderViewProps> = ({ moduleName }) => (
  <div className="p-6">
...
const ModuleCard: React.FC<ModuleCardProps> = ({ title, description, status }) => (
  <div className="bg-white border rounded-lg p-4 hover:shadow-md transition-shadow">
    <h3 className="font-semibold mb-2">{title}</h3>
    <p className="text-sm text-muted-foreground mb-3">{description}</p>
    <div className={`text-xs font-medium ${
      status === 'implemented' ? 'text-green-600' : 'text-yellow-600'
    }`}>
...
const DashboardView: React.FC = () => (
  <div className="p-6 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <ModuleCard
        title="Continuum Nd:YAG Laser"
        description="Pump Source (TTL Control)"
        status="pending"
      />
      
      <ModuleCard
        title="Daylight MIRcat Laser"
        description="Mid-IR Probe Source"
        status="pending"
      />
      
      <ModuleCard
        title="PicoScope 5244D"
        description="Oscilloscope for Data Collection"
        status="pending"
      />
      
      <ModuleCard
        title="Quantum Composers 9524"
        description="Signal Generator for Synchronization"
        status="pending"
      />
